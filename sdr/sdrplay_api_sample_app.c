...
//short xi_buff[SAMPLE_RATE] = {0};
//short xq_buff[SAMPLE_RATE] = {0};
short IQ[SAMPLE_RATE*2+1] = {0};
int curr = 0, curr_iq=0;

void write_to_disk(){
    //for(int i = 0; i < SAMPLE_RATE; i++){
    //    fwrite(save_samples, "%i,%i\n", xi_buff[i], xq_buff[i]);
    //}
    FILE *save_samples;
    char filename[20];

    snprintf(filename, sizeof(filename), "samples%d.bin", curr_file);
    ...
    printf("[ %f / %i] Data written to file!\n", cpu_time_used, curr_file);
